...
export const createCard = async (req: AuthRequest, res: Response) => {
    try {
        ...
        await logActivity(card.id, userId, 'created card', card.title);
...
export const updateCard = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const changes = [];
        if (parsed.data.title !== undefined) changes.push(`title to "${parsed.data.title}"`);
        if (parsed.data.description !== undefined) changes.push('description');
        if (parsed.data.priority !== undefined) changes.push(`priority to "${parsed.data.priority}"`);
        if (parsed.data.dueDate !== undefined) changes.push(`due date to ${parsed.data.dueDate ? new Date(parsed.data.dueDate).toLocaleDateString() : 'none'}`);
        if (parsed.data.listId !== undefined && parsed.data.listId !== card.listId) changes.push('list');
        const details = changes.length > 0 ? `Changed ${changes.join(', ')}` : undefined;
        await logActivity(cardId, userId, 'updated card', details);
